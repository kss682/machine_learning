...
class NN():
	...
	def layers(self,X_train,y_train):
		...
		hiddenlayerweights=np.random.randint(1,size=(4,50))
		hiddenlayerbias=np.ones([150,50])
		outputlayerweights=np.random.randint(1,size=(50,1))
		outputlayerbias=np.ones([150,1])
		...
		for i in range(100000000):
			...
			#print(y_train.shape)
			print(output.shape)			
			output_error=np.subtract(y_train,output)
			...
			print(outputlayerweights.shape)
